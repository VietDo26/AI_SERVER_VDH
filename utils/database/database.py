...
def main():
    ...
    # print(data_media)
    create_table('/home/viet/workspace/AI_SERVER_VDH/utils/database/database.db')
    insert_data_into_role_table()
    data_media = select_data_table(table='media')
    print(data_media)
    data_role = select_data_table(table='role')
    print(data_role)
...
def insert_data_into_media_table(id:int ,user_id:int, type:str, input_image_path:str, input_video_path:str, input_voice_path:str, output_video_path:str, size:str, time:str):
    try:
        conn = sqlite3.connect(os.path.join(PATH,"database.db"))
        c = conn.cursor() 
        # Insert data into the role table
        c.execute('INSERT INTO media (id, user_id, type, input_image_path, input_video_path, input_voice_path, output_video_path, size, time) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?)', \
            (id,user_id, type, input_image_path, input_video_path, input_voice_path, output_video_path,size,time))
        # (1, 'video', '/path/to/image', '/path/to/video', '/path/to/voice', '/path/to/output', '500MB', '12:00:00'))
        conn.commit()
        print("[INFO] : insert data into table media of database.")
    except Error as e:
        print(e)
    finally:
        if conn:
